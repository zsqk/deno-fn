...
function QRCodeModel(this: any, typeNumber: number, errorCorrectLevel: number) {
...
QRCodeModel.createBytes = function (
...
var QRUtil = {
  ...
  getMask: function (maskPattern: string, i: number, j: number) {
    switch (maskPattern) {
  ...
  getLengthInBits: function (mode: string, type: string | number) {
    if (1 <= type && type < 10) {
      switch (mode) {
...
function QRPolynomial(this: any, num: string | any[], shift: string | number) {
...
QRPolynomial.prototype = {
  ...
  multiply: function (
    e: { getLength: () => number; get: (arg0: number) => any },
  ) {
...
/** Constructor */
function QRCode(this: any, options: { [x: string]: any; content?: string }) {
  var instance = this;
  ...
  //Gets text length
  function _getUTF8Length(content: string | number) {
    var result = encodeURI(content).toString().replace(
      /\%[0-9a-fA-F]{2}/g,
      'a',
    );
...
QRCode.prototype.svg = function (opt: { container: any }) {
  ...
  var predefined = typeof options.predefined != 'undefined'
    ...
    : false;
  var defs = predefined
    ? indent + '<defs><path id="qrmodule" d="M0 0 h' + ysize + ' v' + xsize +
      ' H0 z" style="fill:' + options.color +
      ';shape-rendering:crispEdges;" /></defs>' + EOL
    : '';

  //Background rectangle
  var bgrect = indent + '<rect x="0" y="0" width="' + width + '" height="' +
    height + '" style="fill:' + options.background +
    ...
    for (var x = 0; x < length; x++) {
      ...
      if (module) {
        ...
        if (join) {
          ...
          pathdata += 'M' + px + ',' + py + ' V' + h + ' H' + w + ' V' + py +
            ' H' + px + ' Z ';
        } else if (predefined) {
          //Module as a predefined shape, thanks to @kkocdko
          modrect += indent + '<use x="' + px.toString() + '" y="' +
            py.toString() + '" href="#qrmodule" />' + EOL;
        } else {
          //Module as rectangle element
          modrect += indent + '<rect x="' + px.toString() + '" y="' +
            py.toString() + '" width="' + xsize + '" height="' + ysize +
  ...
  if (join) {
    modrect = indent + '<path x="0" y="0" style="fill:' + options.color +
...
QRCode.prototype.save = function (
  file: any,
  callback: (error: any, result: any) => void,
) {
  var data = this.svg();
  if (typeof callback != 'function') {
    callback = function (error: any, result: any) {};
  }
  try {
